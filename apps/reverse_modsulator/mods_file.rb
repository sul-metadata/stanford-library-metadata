require 'nokogiri'
require_relative 'reverse_modsulator'
...
# not included: part, geo extension
  ...
  def process_mods_elements(mods, template, xpath_root)
    ...
    # Languages
    mods_language_nodes = mods.xpath("#{xpath_root}#{@ns}:language")
    template_language_nodes = template.xpath("#{xpath_root}#{@ns}:language")
    output.merge!(extract_languages(mods_language_nodes, template_language_nodes))
    # Subjects
  ...
  # Extract the data and attribute values for language code/text subelements that may be paired.
  # Elements are still processed if only one of the pair is present.
  # @param [Nokogiri::Node] mods_nodes       The set of data nodes to be processed.
  # @param [Nokogiri::Node] template_nodes   The corresponding template nodes.
  # @return [Hash]                          Key: header code; value: metadata value.
